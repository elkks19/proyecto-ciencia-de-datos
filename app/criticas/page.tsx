...
import { getCriticalImpactStats, MOVIE_MOCKS } from "../data/movie-mocks";

const criticalStats = getCriticalImpactStats();

const highImpactCount = criticalStats.filter(
  (item) => item.impactLevel === "High"
).length;
const averageTomatometer = Math.round(
  criticalStats.reduce((sum, item) => sum + item.tomatometerRating, 0) /
    criticalStats.length
);
const averageAudience = Math.round(
  criticalStats.reduce((sum, item) => sum + item.audienceRating, 0) /
    criticalStats.length
);
...
const impactDistribution = [
  {
    label: "High",
    count: criticalStats.filter((item) => item.impactLevel === "High").length,
    color: "#B35C4B",
  },
  {
    label: "Medium",
    count: criticalStats.filter((item) => item.impactLevel === "Medium").length,
    color: "#D9C393",
  },
  {
    label: "Low",
    count: criticalStats.filter((item) => item.impactLevel === "Low").length,
    color: "#8DAA91",
  },
] as const;
...
const pieStops = impactDistribution
  ...
  .reduce(
    (acc, item) => {
      ...
      const portion = (item.count / MOVIE_MOCKS.length) * 100;
      ...
      <div className="flex w-full flex-col gap-8 px-4 py-5 sm:px-6 lg:px-8 xl:px-10 xl:py-8">
            ...
            <div className="space-y-5 p-6 sm:p-8">
              ...
              <div className="max-w-4xl space-y-3">
                ...
                <p className="max-w-3xl text-sm leading-7 text-[#B5B0A6] sm:text-base">
                  Esta pagina traduce campos del CSV como `critics_consensus`,
                  `tomatometer_rating`, `audience_rating`, volumen critico y
                  estatus en una lectura de impacto para producto y ciencia de
                  datos.
                </p>
              </div>
            </div>
          ...
          <Card className="rounded-[2rem] border border-[#3A3C45] bg-[#17181E] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.24)]">
            <div className="space-y-4">
              ...
              <div className="grid gap-6 lg:grid-cols-[220px_1fr] lg:items-center">
                <div className="mx-auto flex h-[220px] w-[220px] items-center justify-center rounded-full border border-[#3A3C45] bg-[#111217]">
                  <div
                ...
                <div className="space-y-4">
                  {impactDistribution.map((item) => (
                    <div
                      key={item.label}
                      className="flex items-center justify-between rounded-[1.2rem] border border-[#3A3C45] bg-[#111217] px-4 py-3"
                    >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {criticalStats
                  ...
                  .sort((left, right) => {
                    const leftScore =
                      left.tomatometerRating + left.audienceRating;
                    const rightScore =
                      right.tomatometerRating + right.audienceRating;
                  ...
                  .map((item, index) => (
                    <div
                      key={item.id}
                      className="flex items-center justify-between gap-4 rounded-[1.4rem] border border-[#3A3C45] bg-[#111217] px-4 py-3"
                    >
                      ...
                      <span className="font-mono text-sm text-[#F3EBDD]">
                        {item.tomatometerRating + item.audienceRating}
                      </span>
                    </div>
                  ))}
              </div>
        ...
        <section className="grid gap-4 xl:grid-cols-2">
          {criticalStats.map((item) => {
            const movie = MOVIE_MOCKS.find((entry) => entry.id === item.id);

            return (
              <Card
                key={item.id}
                className="rounded-[2rem] border border-[#3A3C45] bg-[#17181E] p-5 shadow-[0_18px_40px_rgba(0,0,0,0.22)]"
              >
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-[#B5B0A6]">
                          {movie?.rottenTomatoes.productionCompany} /{" "}
                          {movie?.rottenTomatoes.originalReleaseDate}
                        </p>
                      </div>
